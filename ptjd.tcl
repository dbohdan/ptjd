...
# Copyright (c) 2017, 2020, 2024 D. Bohdan and contributors listed in AUTHORS.
# License: MIT.
...
# Escape unprintable binary data in $s for printing in error messages.
...
# A [format %0${width}b $x] substitute for Tcl 8.5.
...
# Take a table of values and return a dictionary
# mapping a Huffman code to each of the values
# The Huffman codes are represented as binary digits.
...
# [binary scan] the data in the variable $data (in the caller's scope)
# starting at the offset $ptr (in the same scope)
# and store the results in the variables (ditto) listed in $args.
...
# The high nibble is shifted to the left.
...
# - quantization tables
# - Huffman tables
# - restart interval definitions
# - APP sections
# - and comments
# from $data starting at $ptr until $until is encountered.
# Return a list containing the new values for
# $ptr, $qts, $huffdc, $huffac, and $ri.
            ...
            default {
                if {$marker eq $until} {
                    break
                          ...
                          || ($marker eq "\xFF\xFE")} {
                    # Skip APP0-APPF sections and comments.
                    # APP0 is the JFIF header,
                    # APP1 is the EXIF header,
                    # APPE is Adobe info.
...
proc ::ptjd::read-frame-header {data ptr} {
    ...
    return [list \
        $ptr \
        [dict create p $p y $y x $x nf $nf components $components] \
    ]
}
...
proc ::ptjd::read-scan-header {data ptr} {
    ...
    return [list \
        $ptr \
        [dict create \
            ns $ns components $components ss $ss se $se ah $ah al $al \
        ] \
    ]
}

# Read $n bits from $bits.
# If there aren't $n bits in $bits,
# read enough bytes from $data starting at $ptr into $bits
# and advance $ptr accordingly.
# Escaped \xFF values are accounted for.
# Return the updated $ptr and $bits, plus the read bits
# ($n bits as individual list elements).
...
# Read one Huffman code from $data.
...
# per the JPEG standard.
# See "Table 5 - Huffman DC Value Encoding" at
# https://web.archive.org/web/20190118205958/https://www.impulseadventure.com/photo/jpeg-huffman-coding.html
...
# given the matching number of pixels,
# the scaling factor,
# and the maximum horizontal/vertical scaling factor
...
# $hor and $vert are the horizontal and vertical block count respectively,
# and $h and $v are the corresponding scaling factors.
...
# Crudely upscale a color plane by duplicating values.
# Each of $scaleH and $scaleV should be
# either 1 (no upscaling) or 2 (upscale in this direction).
...
# Upscale a color plane somewhat less crudely.
# Each of $scaleH and $scaleV should be
# either 1 (no upscaling) or 2 (upscale in this direction).
